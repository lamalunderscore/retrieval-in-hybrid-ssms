...
from transformers.cache_utils import Cache, DynamicCache  # we need __iter__ and __len__ of pkv
...
# attn_weights.shape: torch.Size([1,          32,        33,      33             ])
#                                 batch_size, num_heads, out_len, sequence_length

# attn_output.shape: torch.Size([1,          32,        33,      128            ])
#                                batch_size, num_heads, out_len, sequence_length
# attn_weights.shape: torch.Size([1, 32, 1, 34])
# attn_output.shape: torch.Size([1, 32, 1, 128])
...
def get_topk(
    ...
    head_mask_recorder: AttentionRecorder | None = None,
) -> torch.Tensor | None:
...
class HybridMambaAttentionDynamicCache(DynamicCache):
    ...
    def to_legacy_cache(self) -> tuple[tuple[torch.Tensor], tuple[torch.Tensor]]:  # type: ignore
    ...
    def from_legacy_cache(
        cls, past_key_values: tuple[tuple[torch.FloatTensor, torch.FloatTensor]] | None = None
    ) -> "DynamicCache":
        raise NotImplementedError(
            "HybridMambaAttentionDynamicCache does not have a legacy cache equivalent."
        )
...
class JambaAttention(nn.Module):
    ...
    def forward(
        ...
        past_key_value: HybridMambaAttentionDynamicCache | None = None,
        output_attentions: bool | None = False,
        use_cache: bool | None = False,
        cache_position: torch.LongTensor | None = None,
    ) -> tuple[torch.Tensor, torch.Tensor | None, HybridMambaAttentionDynamicCache | None]:
...
class JambaMambaMixer(nn.Module):
    ...
    # fmt: off
    def slow_forward(self, input_states, cache_params: HybridMambaAttentionDynamicCache|None = None, attention_mask: Optional[torch.LongTensor] = None):
...
class JambaAttentionDecoderLayer(GradientCheckpointingLayer):
    ...
    def forward(
        ...
        """Forward call for decoder layer.
...
class JambaForCausalLM(JambaPreTrainedModel, GenerationMixin):
    ...
    def _set_sparse_attributes(
        ...
        for i, layer in enumerate(self.model.layers):
            if self.config.layers_block_type[i] == "attention":
